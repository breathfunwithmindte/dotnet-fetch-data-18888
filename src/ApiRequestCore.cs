...
namespace Turism.DataFetch
{
  ...
  public abstract class ApiRequestCore
  {
    ...
    protected abstract void StoreData (string fileName, List<Dictionary<string, object>> data); // will store that data to local file as := value1 <%> value2 <%> ... <%> valueN    -- for each element of the list
