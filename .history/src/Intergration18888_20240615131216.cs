  ...
  public class Intergration18888 : ApiRequestCore
  {
    ...
    public async Task Main (Intergration18888Configuration config)
    {
      if(config.FetchType == DataFetchTypes.PREFECTURES_AND_LOCATIONS)
      {
        ...
        var something = await this.HttpGet<PrefectureLocationResponse>(config.FetchPath, new Dictionary<string, string>());
        Console.WriteLine(JsonSerializer.Serialize(something, new JsonSerializerOptions{ Encoder = JavaScriptEncoder.Create(UnicodeRanges.BasicLatin, UnicodeRanges.GreekandCoptic)}));
